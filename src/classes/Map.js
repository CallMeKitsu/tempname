export class GameMap {
  constructor(scene, tilesets_names, tilemap_name) {

    console.log(tilesets_names)

    this.scene = scene
    this.tilesets_names = tilesets_names
    
    for (var tileset of tilesets_names) {
      scene.load.image(tileset, `assets/tilesets/${tileset}.png`)
      scene.load.tilemapTiledJSON('map', `assets/maps/${tilemap_name}.json`)
    }
  }

  build(layers) {
        
    const map = this.scene.make.tilemap({key: "map", tileWidth: 16, tileHeigth: 16})
    ...
    for (var object of map.getObjectLayer('hitboxes').objects) {
      let rect = this.scene.add.rectangle(object.x + (object.width/2), object.y + (object.height/2), object.width, object.height, 0x6666ff, 0)
